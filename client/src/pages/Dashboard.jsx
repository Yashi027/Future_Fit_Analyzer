              ...
              {roadmap.map((skill) => {
                ...
                return(
                  ...
                  className={`aspect-square rounded-2xl flex flex-col items-center justify-center p-2 text-center transition-all border ${
                    isMastered ? 'bg-indigo-600 text-white border-indigo-600shadow-md'
                ...
                {masteredCount < totalSkills ? (
                  <p className='text-slate-800 max-w-2xl'>
                    ...
                    your immediate priority is mastering <span className='text-indigo-500 font-bold'>
                      {roadmap.find(s => (skillRatings[s.name] || 0)<4)?.name}
                    </span>.
                    Focus on building one production grade project using this tech to move the needle.
                  </p>
                ) : (
